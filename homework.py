...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        calories = ((RUN_COEFF_CALORIE_1 * speed - RUN_COEFF_CALORIE_2)
                    * self.weight / M_IN_KM
...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        calories = ((self.WLK_COEFF_CALORIE_1 * self.weight
...
class Swimming(Training):
    ...
    def get_spent_calories(self) -> float:
        speed = (self.length_pool * self.count_pool
                 / self.M_IN_KM / self.duration)
        calories = ((speed + self.SWM_COEFF_CALORIE_1)
...
def read_package(training_type: str, data: List[int]) -> Training:
    ...
    if training_type not in training_deff:
        print('Error: Такой вид спорта не обрабатывается.')
    else:
